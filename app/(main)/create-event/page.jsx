...
const eventSchema = z.object({
  ...
  themeColor: z.string().default("#FADCA2"),
});
...
const CreateEvent = () => {
  ...
  // Color presets
  const colorPresets = [
    "#FADCA2", // Default color (always available)
    ...(hasPro ? ["#FFC9C1", "#95B9AC", "#CB9FD2", "#00B6FF", "#1e3a8a"] : []),
  ];

  const handleColorClick = (color) => {
    if (color !== "#FADCA2" && !hasPro) {
  ...
  const onSubmit = async (data) => {
    ...
    //   // Check if trying to use custom color without Pro
      if (data.themeColor !== "#FADCA2" && !hasPro) {
        setUpgradeReason("color");
        setShowUpgradeModal(true);
        return;
      }
  ...
  return (
    ...
    >
      ...
      <div className="max-w-6xl mx-auto grid md:grid-cols-[320px_1fr] gap-10">
        {/* left sidebar -img+theme */}
        <div className="space-y-6">
            <div className="aspect-square w-full rounded-xl overflow-hidden flex items-center justify-center cursor-pointer border-2"
